      ...
      <nav className='bg-white dark:bg-black shadow-lg dark:shadow-slate-800'>
         <div className='flex items-center font-medium justify-between'>
            <div className='z-50 p-5 md:w-auto w-full flex justify-between ml-4'>
              <h2 className='cursor-pointer h-9 px-4 pl-6 dark:text-yellow-200'><span className='font-bold text-3xl '>T</span>eam<span className='font-bold'>F</span>inder</h2>
              <div className='text-3xl md:hidden cursor-pointer transition-all dark:text-white' onClick={()=>dispatch(setOpen(!navState.open))}>
             ...
             {!check.isLogin ? (<>
                        ...
                        <div className=' text-center' >
                          ...
                          <div className=' pr-5 cursor-pointer' id='profile' onClick={()=>setProfile(!profile)}>
                             <span className=' text-3xl dark:text-white'><ion-icon name="person-circle-outline"></ion-icon></span>
                             <div className=' text-sm text-slate-500 '>{check.myUser.firstName}</div>
                          </div>
                          <div className={`dark:bg-black bg-white absolute top-20 py-4 px-2 right-7 rounded-lg z-50 ${profile ? 'block' : 'hidden'} transition-all`} >
                             <div className='dark:text-white text-sm py-2  hover:text-blue-700 cursor-pointer' onClick={()=>{setProfile(!profile);navigate("/viewProfile")}}>Edit Profile</div>
                             <div className='dark:text-white text-sm py-1 hover:text-blue-700 cursor-pointer' onClick={()=>{setProfile(!profile);handleLogout()}}>Logout</div>
                          </div>
                        </div>
                    </>}
            </div>
            ...
            `}>
              <li>
                <div className={`dark:text-white py-3 ml-3 max-w-max cursor-pointer relative block overflow-hidden after:absolute after:content-[''] ${pathMap.home?"after:left-0 after:from-blue-200 after:to-blue-500":'after:-left-full after:from-indigo-400 after:to-indigo-700'} after:bg-gradient-to-l after:transition-all hover:after:left-0 after:bottom-0 after:w-full after:h-[2px]`} onClick={()=>{
                  dispatch(setOpen(false));
                  navigate("/");
                }}>
                  Home
                </div>
                <div className={`dark:text-white py-3 ml-3 max-w-max cursor-pointer block relative overflow-hidden after:absolute after:content-[''] ${pathMap.dashboard?"after:left-0 after:from-blue-200 after:to-blue-500":'after:-left-full after:from-indigo-400 after:to-indigo-700'} after:bg-gradient-to-l after:transition-all hover:after:left-0 after:bottom-0 after:w-full after:h-[2px]`} onClick={()=>{
                  dispatch(setOpen(false));
                  navigate("/dashboard");
                }}>
                  Dashboard
